...
function setupEventListeners(): void {
  ...
  eventBus.on('document-changed', () => {
    scheduleSessionSync();
    window.parent?.postMessage({ type: 'rhwp-studio-event', event: 'document-changed' }, '*');
  });
...
function setupSessionSync(): void {
  const params = new URLSearchParams(window.location.search);
  const rawSyncUrl = params.get('syncUrl');
  if (!rawSyncUrl) return;
  try {
    sessionSyncUrl = new URL(rawSyncUrl, window.location.href).toString();
  } catch {
    sessionSyncUrl = rawSyncUrl;
  }
}

function scheduleSessionSync(delayMs = 1500): void {
  if (!sessionSyncUrl || wasm.pageCount <= 0) return;
  if (sessionSyncTimer !== null) {
...
async function syncSessionNow(): Promise<void> {
  if (!sessionSyncUrl || wasm.pageCount <= 0) return;
  if (sessionSyncInFlight) {
    sessionSyncQueued = true;
    return;
  }
  sessionSyncInFlight = true;
  sessionSyncQueued = false;
  try {
    const bytes = wasm.exportHwp();
    const response = await fetch(sessionSyncUrl, {
      method: 'PUT',
    ...
    window.parent?.postMessage({ type: 'rhwp-studio-event', event: 'session-synced' }, '*');
  } catch (error) {
    console.warn('[session-sync] 실패:', error);
    window.parent?.postMessage({
      type: 'rhwp-studio-event',
      event: 'session-sync-error',
      error: error instanceof Error ? error.message : String(error),
    }, '*');
  } finally {
    sessionSyncInFlight = false;
    if (sessionSyncQueued) {
...
window.addEventListener('message', async (e) => {
  ...
  try {
    switch (method) {
      ...
      case 'exportHwp': {
        const bytes = wasm.exportHwp();
        reply({ data: Array.from(bytes), fileName: wasm.fileName });
        break;
      }
      case 'syncSession':
